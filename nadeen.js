...
if (!fs.existsSync(df)) {
  if (config.SESSION_ID) {
    const sessdata = config.SESSION_ID.replace('NADEEN-MD&', '')
    ...
    try {
      ...
      if (_0x332a35) {
        return (
          ...
          await _0x259b92[_0x3d629c(353)](_0x739219, {
            text: '*You are banned by NADEEN TEAM \u274C*',
          })
        )
      }
...
app.get('/', (_0x24c12e, _0x4dc848) => {
  _0x4dc848.send('\uD83D\uDCDF NADEEN DL Working successfully!')
})
app.listen(port, () =>
  console.log(
    'Movie-NADEEN-Md Server listening on port http://localhost:' + port
  )
)
setTimeout(() => {
  connect()
